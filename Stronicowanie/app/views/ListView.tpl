...
<div class="table-wrapper">
    ...
    {if $pagination->page > 1}
        <a href="{$conf->action_url}showAll/{$pagination->page - 1}{if $filtr->login != ''}/{$filtr->login}{/if}" class="button small"><<</a>
    {/if}
        {for $i = 0 to ($pagination->max - 1)}
            <a href="{$conf->action_url}showAll/{$i+1}{if $filtr->login != ''}/{$filtr->login}{/if}" class="button small">{$i+1}</a>        
        {/for}
